...
function setupWebGL(
  ...
  const draw = () => {
    ...
    gl.uniform2f(gl.getUniformLocation(dispProg, 'uMouse'), mouseRef.current[0], mouseRef.current[1]); // mouse y is naturally flipped in vertex shader mapping
...
export function MissionVisionSection() {
  ...
  // Unsplash URLs for Mission and Vision
    ...
    const ctx = gsap.context(() => {
      ScrollTrigger.create({
        ...
        onUpdate: (self) => {
          ...
          const eased = gsap.utils.clamp(0, 1, gsap.parseEase("power2.inOut")(p * 1.5)); // fast reveal
          
          if (leftPanelRef.current) {
            // Expand from center: start clipped 100% on left side, revealing towards left
            leftPanelRef.current.style.clipPath = `inset(0 0 0 ${100 - eased * 100}%)`;
          }
          if (rightPanelRef.current) {
            // Expand from center: start clipped 100% on right side, revealing towards right
            rightPanelRef.current.style.clipPath = `inset(0 ${100 - eased * 100}% 0 0)`;
          }
        }
      });
    }, sectionRef);
  ...
  const initLeftWebGL = useCallback(() => {
    ...
    leftGlStateRef.current = setupWebGL(
  ...
  const initRightWebGL = useCallback(() => {
    ...
    rightGlStateRef.current = setupWebGL(
  ...
  return (
    <section ref={sectionRef} className="relative z-20 h-[200vh] bg-surface">
      <div className="sticky top-0 h-screen w-full flex overflow-hidden">
        ...
        >
          ...
          <div className="absolute inset-0 flex flex-col justify-center px-12 md:px-24 pointer-events-none">
             <div className="text-white/90 max-w-lg ml-auto">
