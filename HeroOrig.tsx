...
// ΓöÇΓöÇ WebGL shaders for fluid cursor displacement ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
const VERT_SRC = `
  attribute vec2 aPos;
  varying vec2 vUv;
  void main() {
...
// Pass 1 ΓÇô update displacement texture (ping-pong)
const DISP_FRAG_SRC = `
...
// Pass 2 ΓÇô render video through displacement
...
// Each image: 230├ù310 px  |  within-pair gap: 10 px  |  between-pair gap: 16 px
// 6 pairs per strip ΓåÆ CYCLE = 6├ù(230+10+230) + 5├ù16 + 16 = 2916 px
const CYCLE_PX = 2916;
...
function pickDissimilarPairs(
  ...
  // Rare edge case: not enough dissimilar products ΓÇö fill without repeating names.
...
// About panel runs from ABOUT_START_VH ΓåÆ ABOUT_START_VH + 2 viewports.
...
export function HeroSection() {
  ...
  const glStateRef   = useRef<{
    ...
    quad:        WebGLBuffer;
    ping:        number; // 0 or 1 ΓÇö which FBO is the "current" target
    mouse:       [number, number];
  ...
  // ΓöÇΓöÇ GSAP setup ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      // ΓöÇΓöÇ Main scroll-driven animation ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
      ...
      const proxyTween = gsap.to(scrollProxy, {
        ...
        paused: true,
        onUpdate: () => {
          const p = scrollProxy.p; // 0 ΓåÆ 1

          // ΓöÇ Phase 1 (p: 0 ΓåÆ 0.45): de-rotate strips, fade hero text ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
          ...
          // ΓöÇ Phase 2 progress ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
          const p2     = Math.max(0, (p - 0.45) / 0.55);
          const p2card = Math.max(0, (p2 - 0.35) / 0.65);

          // One-shot center-pair detection & freeze ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
          if (p > 0.005 && !hasFoundCenterRef.current) {
            ...
            if (strips.length > 1) {
              ...
              // Pick random extras ΓÇö different from initial and not visually similar
          ...
          // Revert if scrolled back to top ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
          if (p <= 0.005 && hasFoundCenterRef.current) {
          ...
          // Apply Phase 1 transforms ΓÇö runs every frame unconditionally ΓöÇΓöÇΓöÇΓöÇΓöÇ
          ...
          // ΓöÇ Phase 2 (p: 0.45 ΓåÆ 1.0): expand clone pair + reveal card ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
      ...
      // ΓöÇΓöÇ Slideshow Wipes & Info Card Swiping (Mersi-Style) ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
      ...
      ScrollTrigger.create({
        ...
        onUpdate: (self) => {
          const sp = self.progress; // 0 ΓåÆ 1
          ...
          // snappy right when the user starts each new product step ΓÇö eliminating
      ...
      // ΓöÇΓöÇ About Panel Reveal (Step 4) ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
      ScrollTrigger.create({
        trigger: scrollContainerRef.current,
  ...
  // ΓöÇΓöÇ WebGL fluid displacement effect ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
  const initWebGL = useCallback(() => {
    ...
    const draw = () => {
      ...
      // Velocity ΓÇö smoothly blend toward raw velocity
      ...
      // ΓöÇΓöÇ Pass 1: update displacement (render into pong) ΓöÇΓöÇ
      ...
      // ΓöÇΓöÇ Pass 2: render video with displacement ΓöÇΓöÇ
  ...
  useEffect(() => {
    ...
    // Pointer tracking ΓÇö store UV coords inside the wrapper
  ...
  return (
    <div ref={scrollContainerRef} style={{ height: `${TOTAL_VH}vh` }}>
      ...
      >
        {/* ΓöÇΓöÇ Diagonal image strips ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ */}
        ...
        {/* ΓöÇΓöÇ High-performance clone for Phase 2 ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ */}
        <div
          ref={clonePairRef}
        ...
        {/* ΓöÇΓöÇ Additional Slideshow Wipes (Mersi-Style) ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ */}
        ...
        {/* ΓöÇΓöÇ About Panel (Step 4: slides in after all products) ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ */}
        <div style={{ position: 'absolute', inset: 0, zIndex: 20, pointerEvents: 'none' }}>

         {/* LEFT: green glassmorphic panel ΓÇö slides in from left */}   
          <div
            ref={aboutPanelRef}
          ...
          >
            ...
            >
              ...
              <p className="about-panel-body" style={{ color: "rgba(255,255,255,0.82)" }}>
                ...
                into a fully integrated solutions provider ΓÇö combining over 20 years of industry
          ...
          {/* RIGHT: factory video ΓÇö rendered through WebGL fluid displacement */}
          <div
            ref={aboutVideoWrapperRef}
            className="about-video-wrapper"
            style={{ pointerEvents: 'none', cursor: 'none' }}
          >
            {/* Hidden video ΓÇö only used as WebGL texture source */}
            <video
              ref={aboutVideoRef}
              src={aboutVideoSrc}
              autoPlay
        ...
        {/* ΓöÇΓöÇ Brand-colour overlay (fades on scroll phase 1) ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ */}
        <div ref={overlayRef} className="hero-overlay" aria-hidden="true" />

        {/* ΓöÇΓöÇ Dark scrim for text legibility ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ */}
        <div
          ref={scrimRef}
        ...
        {/* ΓöÇΓöÇ Hero text content ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ */}
        <div
          ref={contentRef}
        ...
        >
          ...
          <p className="hero-animate mt-6 max-w-2xl text-base text-white/70 md:text-lg">
            ...
            showers and automotive windshields ΓÇö processed, finished and installed in Addis Ababa.
          </p>
        ...
        {/* ΓöÇΓöÇ Scroll-driven info card ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ */}
            ...
            {[...Array(1 + EXTRA_STEPS)].map((_, i) => (
              ...
              >
                ...
                <div className="hero-card-text-group" style={{ position: 'absolute', inset: 0 }}>
                  ...
                  {/* Arrow is now decorative ΓÇö the whole card is the link */}
